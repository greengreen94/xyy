...
TARGET_TABLE = "engineer_construction" # engineer_construction，修改1：bid_evaluation_event
...
cursor_src.execute(f"""
    ...
    # where business_type = '政府采购'  # 修改2：WHERE project_name LIKE '%评标%'
""")
...
field_mapping = {
    ...
    'regions': 'ADDRESS', # 修改3：删除regions和industries
...
for batch_num in tqdm(range(total_batches), desc=f"分批插入 {TARGET_TABLE}"):
    ...
    cursor_src.execute(f"""
        ...
        # where business_type = '政府采购' # 修改4：WHERE project_name LIKE '%评标%'
